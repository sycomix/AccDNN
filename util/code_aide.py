...
def code_instance_gen(module_name, blob_din_name, blob_din_en_name, blob_din_rdy_name, blob_din_eop_name, \
    ...
    if param_list:
        ...
        for param in param_list:
            code_str += f'.{param[0]}({param[1]}' + '),\n'
        code_str = code_str[:-2] + ')\n'
        code_str += f'u{str(instance_num)}_{module_name}' + '(\n'
    else:
        code_str += f'{module_name} u{str(instance_num)}_{module_name}' + '(\n'
...
def code_instance_ddr_gen(module_name, blob_din_name, blob_din_en_name, blob_din_rdy_name, blob_din_eop_name, \
    ...
    return (
        f'{module_name} u{str(instance_num)}_{module_name}'
        + '(\n'
        + '.clk(clk),\n'
        + '.rst(rst),\n'
        + '.blob_din('
        ...
        + ddr_read_start_addr_name
        + '),\n'
        + '.dma_engineer_length('
        + ddr_read_req_length_name
        + '),\n'
        + '.dma_engineer_dout('
        + ddr_dout_name
        + '),\n'
...
def code_dma_delay_gen(delays, ddr_data_width=DDR_DATA_WIDTH, instance_num=0):    
    ...
    for dma_idx in range(DDR_DMA_ENGINE_NUM):
        inter_signals[f'layer_ddr_read_req_{str(dma_idx)}'] = 1, 'wire'
        inter_signals[f'layer_ddr_read_length_{str(dma_idx)}'] = 27, 'wire'
        inter_signals[f'layer_ddr_read_addr_{str(dma_idx)}'] = 27, 'wire'
        inter_signals[f'layer_ddr_read_ack_{str(dma_idx)}'] = 1, 'wire'
        inter_signals[f'layer_ddr_dout_{str(dma_idx)}'] = ddr_data_width, 'wire'
        inter_signals[f'layer_ddr_dout_en_{str(dma_idx)}'] = 1, 'wire'
        inter_signals[f'layer_ddr_dout_eop_{str(dma_idx)}'] = 1, 'wire'
    code_str = code_signals_gen(inter_signals)

    module_name = 'ddr_read_delay'
    code_str += module_name + ' #(\n'
    code_str += f'.DDR_DATA_WIDTH({str(ddr_data_width)}' + '),\n'
    for dma_idx in range(DDR_DMA_ENGINE_NUM):
        code_str += f'.delay_req_port{str(dma_idx)}({str(delays[dma_idx])}' + '),\n'
    code_str = code_str[:-2] + ')\n'
    code_str += (
        f'u{str(instance_num)}_{module_name}' + '(\n' + '.clk(clk),\n'
    ) + '.rst(rst),\n'
    for dma_idx in range(DDR_DMA_ENGINE_NUM):
        code_str += (
            (
                        ...
                        (
                            (
                                (
                                    (
                                        (
                                            f'.engine_ddr_read_req_{str(dma_idx)}_o(ddr_read_req_{str(dma_idx)}'
                                            + '),\n'
                                            + '.engine_ddr_read_length_'
                                        )
                                        + str(dma_idx)
                                        + '_o(ddr_read_length_'
                                    )
                                ...
                                + '.engine_ddr_read_addr_'
                            )
                            + str(dma_idx)
                            + '_o(ddr_read_start_addr_'
                        )
                        + str(dma_idx)
                        + '),\n'
                    )
                    + '.engine_ddr_read_ack_'
                )
                + str(dma_idx)
                + '_i(ddr_read_ack_'
            )
                                                            ...
                                                            (
                                                                (
                                                                    (
                                                                        (
                                                                            (
                                                                                f'.layer_ddr_read_req_{str(dma_idx)}_i(layer_ddr_read_req_{str(dma_idx)}'
                                                                                + '),\n'
                                                                                + '.layer_ddr_read_length_'
                                                                            )
                                                                            + str(
                                                                                dma_idx
                                                                            )
                                                                            + '_i(layer_ddr_read_length_'
                                                                        )
                                                                        + str(
                                                                            dma_idx
                                                                        )
                                                                    ...
                                                                    + '.layer_ddr_read_addr_'
                                                                )
                                                                + str(dma_idx)
                                                                + '_i(layer_ddr_read_addr_'
                                                            )
                                                            + str(dma_idx)
                                                            + '),\n'
                                                        )
